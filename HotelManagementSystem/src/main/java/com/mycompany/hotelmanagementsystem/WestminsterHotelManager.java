...
public class WestminsterHotelManager implements HotelManager {
    ...
    public boolean runMenu() {
        ...
        System.out.println("To Edit a guest's name, press 5"); // Added menu option
    ...
    // Add this method to allow editing guest name and surname
    public void editGuestName() {
        ...
        for (int i = 0; i < hotelGuestList.size(); i++) {
            Guest g = hotelGuestList.get(i);
            System.out.println((i + 1) + ". " + g.getName() + " " + g.getSurname() + " (Room: " + g.getRoomNumber() + ")");
        }
        System.out.println("Enter the number of the guest you want to edit:");
        int idx = s.nextInt();
        s.nextLine();
        if (idx < 1 || idx > hotelGuestList.size()) {
            System.out.println("Invalid selection.");
            return;
        }
        Guest guest = hotelGuestList.get(idx - 1);
